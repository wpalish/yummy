...
async def moderate_review(text: str) -> tuple[bool, str]:
    ...
    try:
        verdict = await complete(
            system=(
                "Ты модератор отзывов маркетплейса еды. Ответь ровно одним словом: "
                "OK — если отзыв нормальный (даже негативный, но по делу); "
                "TOXIC — если оскорбления, спам, реклама или нерелевантный текст."
            ),
            user=text,
            max_tokens=5,
            temperature=0,
...
async def generate_box_description(category_ru: str, notes: str) -> str:
    ...
    return await complete(
        ...
        user=f"Категория: {category_ru}. Что обычно внутри (черновик от партнёра): {notes}",
        max_tokens=120,
        temperature=0.8,
    )
