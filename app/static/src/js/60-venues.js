/* ============ ЗАВЕДЕНИЯ (178 точек 6 сетей из 2GIS) ============ */
let ALL_VENUES=null, curChain="all", curVDist="all", vmapObj=null, vmapMode=false;
// logo — настоящий логотип бренда с его официального сайта (self-hosted).
...
function chainBadge(chain, size){
  const m=CHAIN_META[chain]||{cls:"",ic:"☕"};
  const px=size||44;
  if(m.logo) return `<img class="logo img" src="${m.logo}" alt="${esc(chain)}" loading="lazy"
    style="width:${px}px;height:${px}px;border-radius:13px;object-fit:contain;background:#fff;padding:4px" />`;
  return `<div class="logo ${m.cls}" style="width:${px}px;height:${px}px;border-radius:13px;display:grid;place-items:center;font-size:${Math.round(px*0.42)}px;color:#fff">${m.ic}</div>`;
}
...
function successScreen(res){
  ...
  if(o.payment_status==="pending"){ // бокс зарезервирован, ждём оплату (мерчант партнёра)
    if(APIPAY){
      // реальный Kaspi: счёт ушёл в приложение покупателя, оплату подтвердит
      // вебхук ApiPay. Опрашиваем статус заказа, пока не станет paid → QR.
      showModal(`<div class="mc ok-wrap" style="padding-top:1.4rem">
        <div class="ok-ic">⏳</div>
        <h3>Счёт отправлен в ваш Kaspi</h3>
        <p style="color:var(--txt2);font-size:.86rem;margin:.2rem 0 0">${esc(o.partner_name)} · ${esc(o.address)}</p>
        <div class="code">${esc(o.code)}</div>
        <p style="font-size:.84rem;color:var(--txt2);margin:.5rem 0">Откройте приложение <b>Kaspi</b> и подтвердите оплату — счёт на ${money(o.price)} уже там. QR откроется автоматически после оплаты.</p>
        <p id="apStatus" style="font-size:.82rem;color:var(--brown);font-weight:700">Ждём подтверждение оплаты…</p>
        <button class="btn sec" data-act="closeModal" style="margin-top:.5rem">Закрыть (код сохранён)</button>
      </div>`);
      let tries=0;
      const poll=async()=>{
        if(++tries>40)return;   // ~5 минут (инвойс живёт 24ч, но UI не держим вечно)
        try{const fresh=await get("/orders/"+encodeURIComponent(o.code));
          if(fresh.payment_status==="paid"){ successScreen({order:fresh,qr_svg:qrTag(fresh.code)}); loadOrders&&loadOrders(); return; }
        }catch(e){}
    ...
    // демо: оплату подтверждает админ/вебхук; покупателю показываем ожидание
    showModal(`<div class="mc ok-wrap" style="padding-top:1.4rem">
      <div class="ok-ic">⏳</div>
      <h3>Бокс зарезервирован на 15 минут</h3>
      <p style="color:var(--txt2);font-size:.86rem;margin:.2rem 0 0">${esc(o.partner_name)} · ${esc(o.address)}</p>
      <div class="code">${esc(o.code)}</div>
      <p style="font-size:.84rem;color:var(--txt2);margin:.5rem 0">Оплата зачисляется напрямую заведению. QR откроется после подтверждения оплаты.</p>
...
// QR из кода выдачи на клиенте — сырой <svg> (тот же контракт, что backend qr_svg).
// Пусто, если qrcode-generator не загружен. Оборачивать в .qr — на месте вызова.
function qrTag(code){
  try{ if(typeof qrcode==="function"){const q=qrcode(0,"M");q.addData(code);q.make();
    return q.createSvgTag({cellSize:6,margin:2});} }catch(e){}
  return "";
}
window.showCode=code=>{
  const svg=qrTag(code); const qr=svg?`<div class="qr">${svg}</div>`:"";
