...
const cookieValue=name=>document.cookie.split("; ").find(x=>x.startsWith(name+"="))?.split("=").slice(1).join("=")||"";
const csrfToken=()=>decodeURIComponent(cookieValue("__Host-yummy_csrf")||cookieValue("yummy_csrf"));
async function api(m,u,b,_retry){const h=b!==undefined?{"Content-Type":"application/json"}:{};
  if(!["GET","HEAD","OPTIONS"].includes(m)){const csrf=csrfToken();if(csrf)h["X-CSRF-Token"]=csrf;}
  const r=await fetch(u,{method:m,headers:h,credentials:"same-origin",body:b!==undefined?JSON.stringify(b):undefined});
  const a=account();
  if(r.status===401&&!_retry&&a&&a.server&&!u.startsWith("/session/")){
    try{const csrf=csrfToken(),rh=csrf?{"X-CSRF-Token":csrf}:{};
      const rr=await fetch("/session/refresh",{method:"POST",headers:rh,credentials:"same-origin"});
      if(rr.ok)return api(m,u,b,true);}catch(e){}
  }
...
async function openBox(id){
  ...
  showModal(`<div class="mh" style="background-image:url(/static/img/${imgFor(b)})"><button class="x" data-action="close">✕</button></div>
    <div class="mc">
