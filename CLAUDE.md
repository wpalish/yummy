...
1. **index.html И docs/index.html — генерируемые.** Фронт правится в
   ...
   `YUMMY_PAGES_API_BASE=https://yummy-astana.fly.dev make docs` (сам
...
7. **Pages ходит в живой бэкенд, НЕ в демо.** Пересобирать docs всегда с
   `YUMMY_PAGES_API_BASE` — без него `make docs` сбросит витрину в демо-режим и
   на сайте перестанут работать регистрация, вход, заказ и сброс пароля.
8. **Каталог пуст, пока нет партнёров — это норма.** Витрина показывает честную
   заглушку запуска, а не «ничего не нашлось» (та фраза только для промаха фильтра).
9. **Одна машина на Fly — намеренно.** `_sweeper()` живёт внутри процесса; на
