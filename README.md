...
| 🧑‍🍳 **Партнёр** (кофейня) | создать бокс за минуту, снять бокс с продажи, список своих боксов и броней, **выдача заказа по коду** |
...
- **Отмена** — покупатель может отменить бронь до конца окна самовывоза — бокс возвращается в продажу.
- Протухшие боксы (окно прошло) не показываются в каталоге и не бронируются; refund после окна не возвращает бокс в наличие.

## Доступ и токены

- Кабинет партнёра — заголовок `X-Partner-Token`. Демо-токены: `pt-p1-demo` … `pt-p6-demo`
  (в проде задаются через переменные окружения `PARTNER_TOKEN_P1` и т.д.).
- Админка — заголовок `X-Admin-Token`. Демо-токен: `admin-demo` (переменная `ADMIN_TOKEN`).
- Коды выдачи — 6 символов из криптостойкого алфавита без похожих символов (0/O, 1/I/L) —
  легко продиктовать на кассе, трудно подобрать.

Демо на GitHub Pages (`docs/`) работает без бэкенда (localStorage), поэтому роли там открыты — это витрина флоу, а не продакшн.
