...
function freshState(){
  ...
  return {boxes:SEED_BOXES.map(([id,pid,cat,title,price,val,qty,off,dur,desc])=>{
    ...
    orders:[],reviews:[],templates:[],seq:100};
}
...
function newCode(){return "YM-"+Math.random().toString(16).slice(2,6).toUpperCase();}
function statsCalc(){let gmv=0,issued=0,no_show=0,active=0,refunds=0;
  ST.orders.forEach(o=>{const s=effStatus(o);if(s==="paid"||s==="issued")gmv+=o.price;
  ...
  return {orders_total:total,issued,active,no_show,refunds,gmv,fill_rate:total?Math.round(issued/total*100):0};}
async function _demoGet(u){
  ...
  if(path==="/boxes"){const d=qs.get("district");
    let bs=ST.boxes.filter(b=>b.status==="active"&&b.qty_left>0).map(boxView);
  ...
  if(m=path.match(/^\/orders\/(.+)$/)){const o=ST.orders.find(x=>x.code===decodeURIComponent(m[1]).toUpperCase());if(!o)throw new Error("Заказ не найден");return orderView(o);}
  ...
  if(m=path.match(/^\/partners\/(.+)\/templates$/))
    return (ST.templates||[]).filter(t=>t.partner_id===m[1]).slice().reverse();
...
function demoRecommend(limit=4){
  const mine=myDemoOrders(), avail=ST.boxes.filter(b=>b.status==="active"&&b.qty_left>0).map(boxView);
...
async function _demoPost(u,body){ let m;
  ...
  if(u==="/orders/cancel"||u==="/orders/refund"){const isCancel=u.endsWith("cancel");
    const o=ST.orders.find(x=>x.code===(body.code||"").trim().toUpperCase());
    if(!o)throw new Error("Заказ с таким кодом не найден");
    if(o.status!=="paid")throw new Error(isCancel?"Этот заказ уже нельзя отменить":"Возврат по этому заказу недоступен");
    const started=Date.now()>=Date.parse(o.pickup_from);
    if(isCancel&&started)throw new Error("Окно выдачи уже началось — отмена недоступна");
    if(!isCancel&&!started)throw new Error("Окно выдачи ещё не началось — можно просто отменить бронь");
    o.status=isCancel?"cancelled":"refunded";
    const b=ST.boxes.find(x=>x.id===o.box_id);if(b)b.qty_left++;persist();
    return {ok:true,message:isCancel?"Бронь отменена, бокс вернулся в продажу":"Возврат оформлен — деньги вернутся на карту"};}
  ...
  if(m=u.match(/^\/partners\/(.+)\/templates\/(.+)\/publish$/)){
    const t=(ST.templates||[]).find(x=>x.id===m[2]&&x.partner_id===m[1]);
    if(!t)throw new Error("Шаблон не найден");
    const now=new Date(),to=new Date(now.getTime()+t.hours*3600*1000);
    const b={id:"b"+(ST.seq++),partner_id:t.partner_id,category:t.category,title:t.title,price:t.price,
      value_est:t.value_est,qty_total:t.qty,qty_left:t.qty,pickup_from:now.toISOString(),
      pickup_to:to.toISOString(),description:t.description,created_at:now.toISOString(),status:"active"};
    ST.boxes.push(b);persist();return boxView(b);}
  if(m=u.match(/^\/partners\/(.+)\/templates$/)){
    if(body.value_est<body.price)throw new Error("Ценность должна быть не ниже цены");
    const t={id:"t"+(ST.seq++),partner_id:m[1],category:body.category,title:body.title,price:body.price,
      value_est:body.value_est,qty:body.qty,hours:body.hours,description:body.description,created_at:new Date().toISOString()};
    ST.templates=ST.templates||[]; ST.templates.push(t); persist(); return t;}
  throw new Error("404 "+u);
}
async function _demoDelete(u){let m;
  if(m=u.match(/^\/partners\/(.+)\/templates\/(.+)$/)){
    const before=(ST.templates||[]).length;
    ST.templates=(ST.templates||[]).filter(t=>!(t.id===m[2]&&t.partner_id===m[1]));persist();
    return {deleted:before!==ST.templates.length};}
  throw new Error("404 "+u);
}
